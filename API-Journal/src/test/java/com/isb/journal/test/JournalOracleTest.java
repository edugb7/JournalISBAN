...
public class JournalOracleTest {
    ...
    public void getInfo_ShouldReturnCorrectInfo() throws Exception {
        ...
        mockMvc.perform(get("/journal_operations/oracle/{journal_operation_id}", idJournal)
    ...
    public void putInfo() throws Exception {
        ...
        mockMvc.perform(put("/journal_operations/oracle/{journal_operation_id}", idJournal)
        		.content(jsonEntrada.trim())
        		.headers(httpHeaders)
        		.contentType(MediaType.APPLICATION_JSON))
        		.andExpect(status().isOk());
