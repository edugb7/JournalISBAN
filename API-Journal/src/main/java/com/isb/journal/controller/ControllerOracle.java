...
public class ControllerOracle {
	...
	public Optional<JournalEntityOracle> modidfyEntityOracle(
		...
		Optional<JournalEntityOracle> obj; 

		try {
			obj = dto.findById(id).map( entity -> dto.save(saveChanges(journalEntityOracle, entity)));
		} catch (Exception e) {
			throw new BadRequestError();
		}
	...
	public ResponseEntity<Object> getByIdByOracle(@PathVariable(value="journal_operation_id") UUID id)  {
	...
	public ResponseEntity<Object> getAllByOracle() {
